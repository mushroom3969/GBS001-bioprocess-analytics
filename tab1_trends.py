"""Tab 1: Trend Plots + Feature Comparison with Color Zones"""
...
# ── Helper: color zone plot ───────────────────────────────────────────────────

def plot_feature_comparison(df, feat_x, feat_y, zones, batch_col="BatchID",
                             plot_type="scatter", smooth_method="none", frac=0.3):
    """
    Plot feat_x vs feat_y with background color zones defined by feat_x ranges.

    zones: list of dicts with keys:
        label  : str
        min    : float (inclusive)
        max    : float (inclusive)
        color  : hex string e.g. "#2e86ab"

    plot_type: "scatter" | "dual_line" | "scatter+line"
    """
    # Sort by batch order
    plot_df = df.copy()
    ...
    if smooth_method != "none":
        from utils import smooth_process_data as _smo
        tmp = _smo(plot_df[[feat_x, feat_y]], [feat_x, feat_y],
    ...
    # Assign zone color per point (for scatter)
    def point_color(val):
        for z in zones:
            if z["min"] <= val <= z["max"]:
                return z["color"]
        return "#aaaaaa"  # outside all zones

    point_colors = [point_color(v) for v in x_vals]

    if plot_type == "scatter" or plot_type == "scatter+line":
        fig, ax = plt.subplots(figsize=(10, 6))

        # Draw color zone bands on x-axis
        x_min_data = np.nanmin(x_vals)
        x_max_data = np.nanmax(x_vals)
        y_min_data = np.nanmin(y_vals)
        y_max_data = np.nanmax(y_vals)
        y_pad = (y_max_data - y_min_data) * 0.05

        for z in zones:
            ax.axvspan(z["min"], z["max"], alpha=0.10,
                       color=z["color"], zorder=0)

        if plot_type == "scatter+line":
            ax.plot(x_vals_s, y_vals_s, color="#999999", linewidth=0.8,
                    alpha=0.5, zorder=1)

        sc = ax.scatter(x_vals, y_vals, c=point_colors, s=60,
                        edgecolors="white", linewidths=0.5, zorder=2)

        # Label each point with batch ID
        if batch_col in plot_df.columns:
            for i, (xi, yi, bid) in enumerate(
                    zip(x_vals, y_vals, plot_df[batch_col])):
                ax.annotate(str(bid)[-6:], (xi, yi),
        ...
        # Legend
        legend_handles = [
            mpatches.Patch(color=z["color"], alpha=0.7, label=z["label"])
            for z in zones
        ]
        legend_handles.append(
            mpatches.Patch(color="#aaaaaa", alpha=0.7, label="Outside zones"))
        ...
        # X feature on primary axis
        color1 = "#2e86ab"
        ax1.set_xlabel("Batch Sequence")
        ax1.set_ylabel(feat_x, color=color1)
        ax1.plot(seq, x_vals_s, color=color1, marker="o", ms=4,
                 linewidth=1.5, label=feat_x)
        ax1.tick_params(axis="y", labelcolor=color1)

        # Color zone horizontal bands on primary axis
        for z in zones:
            ax1.axhspan(z["min"], z["max"], alpha=0.12,
                        color=z["color"], zorder=0)

        # Y feature on secondary axis
        ax2 = ax1.twinx()
        color2 = "#e84855"
        ax2.set_ylabel(feat_y, color=color2)
        ax2.plot(seq, y_vals_s, color=color2, marker="s", ms=4,
        ...
        zone_handles = [mpatches.Patch(color=z["color"], alpha=0.5, label=z["label"])
                        for z in zones]
        ax1.legend(lines1 + lines2 + zone_handles, labels1 + labels2 +
                   [z["label"] for z in zones], loc="best", fontsize=8)
...
def render(selected_process_df):
    ...
    with subtabs[1]:
        ...
        st.markdown("選擇兩個特徵，設定 X 軸（或主特徵）的數值區間，以顏色區分不同操作範圍。")
        ...
        cd, ce = st.columns(2)
        smooth_cz = cd.selectbox("平滑（僅雙軸折線）", ["none", "loess", "ewma"],
                                  key="cz_smooth")
        frac_cz = ce.slider("LOESS frac", 0.1, 0.8, 0.3, 0.05,
                             key="cz_frac",
                             disabled=(smooth_cz != "loess"))

        # Show X range for reference
        x_series = selected_process_df[feat_x].dropna()
        st.caption(
            f"**{feat_x}** 的數值範圍：{x_series.min():.3f} ～ {x_series.max():.3f}"
            f"（平均 {x_series.mean():.3f}）"
        )

        # Zone definition
        st.markdown("#### 🎯 設定數值區間")
        st.markdown("每個區間用一個顏色標示。區間可以重疊；點會用最先匹配的區間上色。")

        n_zones = st.number_input("區間數量", min_value=1, max_value=8,
                                   value=3, step=1, key="cz_n_zones")

        PRESET_COLORS = ["#2ecc71", "#f39c12", "#e74c3c",
        ...
        for i in range(int(n_zones)):
            with st.expander(f"區間 {i+1}", expanded=(i < 3)):
                ...
                label = zc1.text_input(f"區間名稱", value=f"Zone {i+1}",
                                        key=f"cz_label_{i}")
                ...
                color = zc4.color_picker("顏色",
                                          value=PRESET_COLORS[i % len(PRESET_COLORS)],
                                          key=f"cz_color_{i}")
        ...
        if st.button("🎨 繪製比較圖", type="primary", key="plot_comparison"):
            # Validate zones
            valid = all(z["min"] < z["max"] for z in zones)
            if not valid:
                st.error("每個區間的最小值必須小於最大值，請重新設定。")
            else:
                with st.spinner("繪圖中..."):
                    try:
        ...
        # Summary table: which batches fall in which zone
        if st.checkbox("顯示各批次所在區間", key="cz_show_table"):
            ...
            summary = selected_process_df[["BatchID", feat_x, feat_y]].copy() \
                if "BatchID" in selected_process_df.columns \
                else selected_process_df[[feat_x, feat_y]].copy()
            ...
            for z in zones:
                ...
                if n > 0:
                    zone_stats.append({
                        ...
                        f"{feat_y} 最小": summary.loc[mask, feat_y].min().round(3),
                        f"{feat_y} 最大": summary.loc[mask, feat_y].max().round(3),
                    })
            if zone_stats:

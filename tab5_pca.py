...
def render(work_df):
    ...
    subtabs = st.tabs(["📊 Scree & Scatter", "🔵 Biplot",
    ...
    with subtabs[0]:
        ...
        with c2:
            ...
            fig, _ = model.scatter(SPE=True, HT2=True)
            st.pyplot(fig); plt.close()
    ...
    with subtabs[1]:
        ...
        fig, _ = model.biplot(n_feat=n_feat_bi, PC=[pc_x, pc_y],
    ...
    with subtabs[2]:
        ...
        ht2_vals = compute_ht2_per_sample(scores, ev)
        thres_68, thres_95, thres_99 = compute_ht2_thresholds(x_scaled.shape[0], n_pc)
        ...
        ax.set_title("Hotelling T² per Batch")
    ...
    with subtabs[3]:
        ...
        top_n       = st.slider("顯示前 N 個特徵", 5, min(30, len(feat_names)), 15,
                                key="pca_top_contrib")

        if view_mode == "所有 PC 的特徵貢獻（總 T²）":
            contribs = compute_total_contribution(scores, loadings, ev, x_scaled, sample_i)
            df_c = pd.DataFrame({"Feature": feat_names, "Contribution": contribs})
            df_c = df_c.reindex(pd.Series(contribs).abs().sort_values(ascending=False).index)
            df_c = df_c.reset_index(drop=True).head(top_n)

            fig, ax = plt.subplots(figsize=(12, max(5, top_n * 0.4)))
            ax.barh(df_c["Feature"], df_c["Contribution"],
                    color=["#e84855" if v > 0 else "#2e86ab" for v in df_c["Contribution"]],
                    alpha=0.85)
            ...
            pc_c   = compute_pc_contribution(scores, loadings, ev, x_scaled, sample_i, sel_pc)
            df_pc  = pd.DataFrame({"Feature": feat_names, "Contribution": pc_c})
            df_pc  = df_pc.reindex(pd.Series(pc_c).abs().sort_values(ascending=False).index)
            df_pc  = df_pc.reset_index(drop=True).head(top_n)

            fig, ax = plt.subplots(figsize=(12, max(5, top_n * 0.4)))
            ax.barh(df_pc["Feature"], df_pc["Contribution"],
                    color=["#e84855" if v > 0 else "#2e86ab" for v in df_pc["Contribution"]],
                    alpha=0.85)
            ax.axvline(0, color="black", lw=1)
